...
import HititInvoiceDocPDF from "./component/HititInvoiceDocPDF";
import {
  ConfirmParent,
  FailedBooking,
  PaymentLoader,
...
export default function HititTicket({ bookingData, query }) {
  ...
  const HititGenerateTicket = useCallback(async () => {
    if (!ticketing && SaleLoading && PayResponse.status === "200") {
      const HititTicketAPI =
        "api/issue-ticket-hitit?pnr=" +
        PNR +
        "&reference_id=" +
        BookingInstance +
        "&price=" +
        bookingData.data.pricing_info.total_amount;
      setTicketing(true);
      setSaleLoading(false);
      Axios.get(HititTicketAPI).then((response) => {
        const res = response.data;
        setTicketResponse(res);
        setTicketing(false);
      });
    }
  }, [ticketing, SaleLoading, PayResponse, PNR, BookingInstance, bookingData]);
  ...
  const InvoiceData = {
    ...
    num_infant: infants,
    price:
      bookingData.data.pricing_info.currency_code +
      " " +
      bookingData.data.pricing_info.TotalPriceWithCommission +
      "/-",
  };
  ...
  function printInvoice() {
    savePdf(
      <HititInvoiceDocPDF InvoiceData={InvoiceData}></HititInvoiceDocPDF>,
                        ...
                        <>
                          <div className="row justify-content-center mb-3">
                            ...
                            <p className="col-sm-12 col-md-12 col-lg-4 text-dark">
                              {bookingData.data.ticket_time_limt}
                            </p>
                          </div>
                          <div className="row  justify-content-center">
                            <div className="row ">
