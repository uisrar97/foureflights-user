  ...
  for (let index = 0; index < passengerArray.length; index++) {
      ...
      if (validity) {
        ...
        if (passengerArray[index].passenger_type === "INF") {
          if (returnDate === "undefined") {
            ...
            if (age >= 730) {
              passengerErrors.dob_month =
                "Infant Age Must Be Less Than 2 Years";
            }
          } else {
            let age = Math.round(
              days_between(
    ...
    if (
      ...
      if (passengerArray[index].passport_number.match(/[^ _-]/g).length < 5) {
        ...
        // if(hasOnlyLtr.test(firstTwo) === false || hasOnlyNmbr.test(lastSeven) === false)
        // {
        //   passengerErrors.passport_number = 'Valid Passport Number format is AA9999999';
        // }
        // // else
        // // {
    ...
    if (
      ...
      passengerArray[index].passenger_type !== "CNN" &&
